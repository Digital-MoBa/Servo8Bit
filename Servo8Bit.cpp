/*
 ...
 modified by Philipp Gahtow 2017
 
 Changelog:
 - 05.11.2017: add support for 16mhz with timer1
 - 10.11.2017: fix detach() function, if called no attach works
...
// *The Servo class - which provides the Arduino-like interface
	...
	#define	TIMSKn  TIMSK //Timer Interrupt Mask Register
    #define TCNTn   TCNT0
    #define OCRnx   OCR0A
    #define OCFnx   OCF0A
	...
	#define	TIMSKn  TIMSK2 //Timer Interrupt Mask Register
    #define TCNTn   TCNT2
    #define OCRnx   OCR2A
    #define OCFnx   OCF2A
    #define OCIEnx  OCIE2A
#endif
...
private:
    ...
    //=============================================================================
    static const    uint16_t          kMaxNumberOfServosSupported = 4;  //The number of servos to support. See NOTE1 below.
...
void ServoSequencer::deregisterServo(uint8_t servoSlotNumber)
{
    ...
    if(servoSlotNumber < kMaxNumberOfServosSupported)
    {
		...
		servoRegistry[servoSlotNumber].pulseLengthInTicks = 128; //add by PG: restore the pulse length to the default setting
    }
    else
    {
...
void ServoSequencer::timerCompareMatchISR()
{
    ...
    case WAITING_FOR_512_MARK:
        ...
        else
        {
			...
			#ifdef USE_TIMER0
            TIFR = (1 << OCFnx);  // write logical 1 to the OCF0A flag to clear it
			...
			#ifdef USE_TIMER2
            TIFR2 = (1 << OCFnx);  // write logical 1 to the OCF0A flag to clear it
                                  // also have to write 0 to all other bits for this to work.
			#endif
...
{
    ServoSequencer::timerCompareMatchISR();
}//end ISR TIM1_COMPA_vect
#endif


//only define this ISR if we are using TIMER2
#if defined(USE_TIMER2)
//=============================================================================
// FUNCTION:    Interrupt service routine for timer2 compare A match
//
// DESCRIPTION: AVR Libc provided function that is vectored into when the
//              timer2 compare A match interrupt fires.
//
// INPUT:       Nothing
//
// RETURNS:     Nothing
...
uint8_t Servo8Bit::attach(uint8_t pin)
{
    ...
    if(myServoNumber == invalidServoNumber) {
		//add by PG: so register and save our servo number
		myServoNumber = ServoSequencer::registerServo();
		if (myServoNumber == invalidServoNumber) {
			//The Servo sequencer is full!
			return 0;
		}
	}
